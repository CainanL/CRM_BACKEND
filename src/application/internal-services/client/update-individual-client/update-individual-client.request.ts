...
export class UpdateIndividualClientAddressRequest {
...
export class UpdateIndividualClientRequest {
    ...
    @ApiProperty({ type: UpdateIndividualClientAddressRequest, description: "Endereço completo" })
    @IsDefined({ message: "O endereço é obrigatório" })
    @ValidateNested()
    @Type(() => UpdateIndividualClientAddressRequest)
    address: UpdateIndividualClientAddressRequest;

    @ApiProperty({ example: "CLI001", description: "Código interno" })
    @IsString()
    @IsNotEmpty()
    internalCode: string;
